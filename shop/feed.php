...
//write
$head = "<?xml version=\"1.0\" encoding=\"utf-8\"?><SHOP>";
$footer = "</SHOP>";
...
while ($db->next_record()) {
    ...
    $towrite .= $s . '<br>';
    fputs($fh, "<PRODUCT>$s</PRODUCT>\n");
    fputs($fh, "<DESCRIPTION>". html_entity_decode($db->get("product_s_desc"), ENT_NOQUOTES , "UTF-8")."</DESCRIPTION>\n");
    $url = "<URL>http://www.cyklosauer.cz/shop/?page=shop/flypage&amp;product_id=". $db->get("product_id") ."&amp;</URL>\n";
    fputs($fh, $url);
    fputs($fh, "<ITEM_TYPE>new</ITEM_TYPE>\n");
    fputs($fh, "<DELIVERY_DATE> ihned </DELIVERY_DATE>\n");
